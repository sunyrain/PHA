...
$RulesVersion = 'standardization_v2_20260707'
...
function Derive-ModelReadyV2 {
  ...
  if ($Blockers.Count -eq 0) { return @($true, 'ready_v2') }
...
function Derive-ManualReviewPriority {
  ...
  if (($ModelReadyV2 -and $Score -ge 2) -or $UnitConverted -or $ComparableClass -eq 'strong' -or ($HighProtein -and $Score -ge 2) -or ($HighMaterial -and $ComparableForReview) -or ($DoiRecordCount -gt 4 -and $ComparableForReview)) {
    return 'high'
  }
...
function Add-DataDictionaryRows {
  param([string]$Path)
  $Existing = @(Import-Csv -LiteralPath $Path | Where-Object { $_.column -notin $StandardFields })
  $NewRows = New-Object System.Collections.Generic.List[object]
  $Tables = @('records_flat.csv', 'records_core.csv', 'model_records.csv', 'model_records_v2.csv', 'inverse_design_seed.csv', 'record_standardization.csv')
...
function Write-RunComparison {
  ...
  $OldModelReady = @($FlatRows | Where-Object { As-Bool $_.model_ready }).Count
  $NewModelReady = @($StandardRows | Where-Object { As-Bool $_.model_ready_v2 }).Count
  ...
  $MetricRows.Add([pscustomobject][ordered]@{ metric = 'model_ready_count'; category = 'legacy_model_ready'; count = $OldModelReady })
  $MetricRows.Add([pscustomobject][ordered]@{ metric = 'model_ready_count'; category = 'model_ready_v2'; count = $NewModelReady })
  $MetricRows.Add([pscustomobject][ordered]@{ metric = 'comparable_numeric_count'; category = 'legacy_primary_target_numeric'; count = $ComparableNumeric })
  ...
  Write-Csv (Join-Path $MetadataDir 'run_comparison_summary_20260707.csv') $MetricRows.ToArray()

  $Lines = New-Object System.Collections.Generic.List[string]
  $Lines.Add('# PHA Incremental Standardization Run Comparison')
  $Lines.Add('')
  $Lines.Add("Generated: $((Get-Date).ToString('yyyy-MM-ddTHH:mm:ss'))")
  $Lines.Add('')
  $Lines.Add('This run is an incremental update to the existing PHA = Protein-Hydrogel Adsorption release. It does not redesign the Q1-Q7 retrieval protocol, record granularity, or legacy output structure.')
  $Lines.Add('')
  ...
  $Lines.Add("| Legacy model_ready | $OldModelReady |")
  $Lines.Add("| model_ready_v2 | $NewModelReady |")
  $Lines.Add("| Legacy comparable numeric | $ComparableNumeric |")
  $Lines.Add('')
  ...
  $Lines.Add('')
  $Lines.Add('## Incremental Fields Added')
  $Lines.Add('')
  foreach ($Field in $StandardFields) {
    $Lines.Add('- `' + $Field + '`: ' + $StandardDescriptions[$Field])
  }
  Set-Content -LiteralPath (Join-Path $MetadataDir 'run_comparison_summary_20260707.md') -Value $Lines -Encoding UTF8
}
...
function Update-SummaryJson {
  ...
  $Summary | Add-Member -NotePropertyName model_ready_v2_records -NotePropertyValue (@($StandardRows | Where-Object { As-Bool $_.model_ready_v2 }).Count) -Force
...
$StandardRows = foreach ($Row in $FlatRows) {
  ...
  $ModelReadyResult = Derive-ModelReadyV2 $Row $ComparableClass $InteractionType
  $ModelReadyV2 = [bool]$ModelReadyResult[0]
  $ModelReadyBlocker = [string]$ModelReadyResult[1]
  $Priority = Derive-ManualReviewPriority $Row $Blob $ComparableClass $ModelReadyV2 $InteractionType ([int]$DoiCounts[$Row.doi])
...
Merge-Standardization $ModelPath $StandardByRecordId
Merge-Standardization $DesignPath $StandardByRecordId

$FlatRows = @(Import-Csv -LiteralPath $FlatPath)
$ModelV2Rows = @($FlatRows | Where-Object { As-Bool $_.model_ready_v2 })
Write-Csv (Join-Path $DataDir 'model_records_v2.csv') $ModelV2Rows
...
Write-RunComparison $FlatRows @($StandardRows) $MetadataDir
Update-SummaryJson (Join-Path $MetadataDir 'summary.json') @($StandardRows)
Update-Checksums $Package

Write-Host "records=$($FlatRows.Count) model_ready_v2=$($ModelV2Rows.Count) strong=$(@($StandardRows | Where-Object { $_.comparable_target_class -eq 'strong' }).Count) medium=$(@($StandardRows | Where-Object { $_.comparable_target_class -eq 'medium' }).Count)"
